...
def user_register(account, password, confirm_password, is_admin):
    if not account:
    ...
    user3 = User(account=account, password=generate_password_hash(password), is_admin=is_admin)
...
def get_users(del_mes=''):
    ...
    if del_mes == '':
        ...
        return jsonify({"userlist": user_list, 'code': 200, 'message': del_mes}), 200


def user_update_user(account):
    if not account:
...
def admin_update_user(id, account):
...
def admin_delete_user(id):
    ...
    if user:
        ...
        db.session.commit()
        return get_users('删除成功')
    else:
        return jsonify({"message": "用户不存在", 'code': 404}), 404


def get_papers(del_mes=''):
    exams = Exam.query.order_by(Exam.id.asc()).all()
    if del_mes == '':
        return jsonify({"message": "读取成功", 'code': 200, 'exams': [exam.as_dict() for exam in exams]}), 200
    else:
        return jsonify({"message": "删除成功", 'code': 201, 'exams': [exam.as_dict() for exam in exams]}), 201
...
def delet_paper(key, id):
    ...
    if exam:
        ...
        db.session.commit()
        return get_papers('删除成功')
    else:
        return jsonify({"message": "试卷不存在", 'code': 404}), 404


def get_questions(del_mes=''):
    ...
    if del_mes == '':
        ...
        return jsonify(
            {"message": "删除成功", 'code': 201, 'questions': [question.as_dict() for question in questions]}), 201
